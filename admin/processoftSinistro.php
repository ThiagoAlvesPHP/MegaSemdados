	...
	if (count($_FILES['arquivo']['tmp_name']) > 0) {
		for ($i=0; $i < count($_FILES['arquivo']['tmp_name']); $i++) {
			if ($_FILES['arquivo']['type'][$i] == 'image/jpeg') {
				...
				imagejpeg($imagem_final, "assets/img/fotos_sinistros/".$arquivo, 100);
				
				$sql->setFTSinistro($num_processo, $arquivo, $texto);
				?>
				<script>
				window.location.href = "processoftSinistro.php?num_processo=<?=$num_processo; ?>";
				</script>
				<?php
			}
		}
...
if (isset($_GET['idImg']) && !empty($_GET['idImg'])) {
	...
	$arquivo = $sql->getFTSinistroID($idImg);
	unlink('assets/img/fotos_sinistros/'.$arquivo['img']);
	$sql->delFTSinistro($idImg);
	?>
	<script>
...
$ft = $sql->getFTSinistro($num_processo);
?>
	...
	<div class="col-sm-10">
		<div class="panel panel-success">
	      ...
	      <div class="panel-body">
	      	...
	      	<hr>
	      	<h3>Fotos Gerais do Sinistro</h3>
	      	<div class="well">
	            		...
	            		<?php
				      	foreach ($ft as $fts) {
				      		...
				      		<tbody>
		            			<tr>
		            				<td>
		            					<a style="font-size: 30px;" href="processoftSinistro.php?num_processo=<?=$num_processo; ?>&idImg=<?=$fts['id']; ?>" class="fas fa-trash-alt"></a>
		            				</td>
		            				<td>
		            					<img src="assets/img/fotos_sinistros/<?=$fts['img']; ?>" style="height: 365px; width: 500px;" class="img-responsive">
		            				</td>
		            				<td>
		            					<style type="text/css">
										#divCarregando {
		            					...
		            					<textarea style="height: 365px;" id="<?=$fts['id']; ?>" name="texto" class="form-control txtftSin"><?=str_replace('.jpg', '', $fts['texto']); ?></textarea>
		            				</td>
		            			</tr>
		            		</tbody>
				      		<?php
