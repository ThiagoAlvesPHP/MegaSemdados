...
//CAPTURANDO FOTOS
$ft = $sql->getFTreposrtagem($num_processo);
	...
	if (count($_FILES['arquivo']['tmp_name']) > 0) {
		for ($i=0; $i < count($_FILES['arquivo']['tmp_name']); $i++) {
			if ($_FILES['arquivo']['type'][$i] == 'image/jpeg') {
				...
				imagejpeg($imagem_final, "assets/img/fotos_reportagem/".$arquivo, 100);
				
				$sql->setFTreportagem($num_processo, $arquivo, $texto);
				?>
				<script>
...
if (isset($_GET['idImg']) && !empty($_GET['idImg'])) {
	...
	$arquivo = $sql->getFTreportagem($idImg);
	unlink('assets/img/fotos_reportagem/'.$arquivo['img']);
	$sql->delFTreportagem($idImg);
	?>
	<script>
	...
	<div class="col-sm-10">
		<div class="panel panel-success">
	      ...
	      <div class="panel-body">
	      	...
	      	<hr>
	      	<h3>Reportagem Fotográfica ( S.O.S )</h3>
	      	<div class="well">
		      	...
		      	<div class="row">
	        		<div class="col-sm-9"></div>
	      			<div class="col-sm-3">
						...
						<a href="processo20.php?num_processo=<?=$num_processo; ?>" class="btn btn-danger">Voltar</a>
						<a href="processo22.php?num_processo=<?=$num_processo; ?>" class="btn btn-primary">Proximo</a>	
	      			</div>
	        	</div>
	        	</form>
		      	<hr>
	            		...
	            		<?php
				      	foreach ($ft as $fts) {
				      		...
				      		<tbody>
		            			<tr>
		            				<td>
		            					<a style="font-size: 20px;" href="processo21.php?num_processo=<?=$num_processo; ?>&idImg=<?=$fts['id']; ?>" class="fas fa-trash-alt"></a>
		            				</td>
		            				<td>
		            					<img src="assets/img/fotos_reportagem/<?=$fts['img']; ?>" style="height: 365px; width: 500px;" class="img-responsive">
		            				</td>
		            				<td>
		            					<style type="text/css">
										#divCarregando {
		            					...
		            					<textarea style="height: 365px;" id="<?=$fts['id']; ?>" name="texto" class="form-control txtP21"><?=str_replace('.jpg', '', $fts['texto']); ?></textarea>
		            				</td>
		            			</tr>
		            		</tbody>
				      		<?php
